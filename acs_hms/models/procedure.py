...
class AcsPatientProcedure(models.Model):
    ...
    def _rec_count_prescription(self):
        Prescription = self.env['prescription.order']
        for rec in self:
            rec.prescription_count = Prescription.search_count([('procedure_id','=',rec.id)])
    ...
    prescription_count = fields.Integer(compute='_rec_count_prescription', string='# Prescriptions')
    ...
    def get_procedure_invoice_data(self):
        ...
        for rec in self:
            ...
            procedure_data = {'product_id': rec.product_id, 'line_type': 'procedure', 'procedure_id': rec.id}
    ...
    def action_prescription(self):
        ...
        action['context'] = {'default_patient_id': self.patient_id.id, 'default_physician_id': self.physician_id.id, 'default_procedure_id': self.id}
        return action
