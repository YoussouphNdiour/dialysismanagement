...
class Appointment(models.Model):
    ...
    medical_history = fields.Text(related='patient_id.medical_history', 
        string="Nephrology Summary", readonly=True)
    ...
    def get_procedure_group_data(self):
        ...
        # Utiliser la date du rendez-vous comme base
        ...
        for line in self.procedure_group_id.line_ids:
            ...
            if nephrology_schedule:
                # Utiliser la méthode get_next_schedule_date du treatment
                treatment_model = self.env['hms.treatment']
                procedure_date = treatment_model.get_next_schedule_date(current_date, nephrology_schedule)
    ...
    @api.constrains('cabin_id', 'date', 'date_to', 'state')
    def _check_cabin_availability(self):
        """Check that the consultation room is not double-booked"""
        for rec in self:
            # Only check if cabin is set and appointment is not cancelled
            if rec.cabin_id and rec.date and rec.date_to and rec.state != 'cancel':
                # Search for overlapping appointments with the same cabin
                domain = [
                    ('id', '!=', rec.id),
                    ('cabin_id', '=', rec.cabin_id.id),
                    ('state', '!=', 'cancel'),
                    ('date', '<', rec.date_to),
                    ('date_to', '>', rec.date),
                ]
                ...
                if overlapping:
                    raise UserError(_(
                        'The consultation room "%s" is already booked for this time slot.\n'
                        'Conflicting appointment: %s (Patient: %s)\n'
                        'Time: %s - %s'
                    ) % (
                        rec.cabin_id.name,
                        overlapping.name,
                        overlapping.patient_id.name,
                        overlapping.date.strftime('%d/%m/%Y %H:%M') if overlapping.date else '',
                        overlapping.date_to.strftime('%d/%m/%Y %H:%M') if overlapping.date_to else ''
                    ))
    ...
    def view_invoice(self):
        appointment_invoices = self.invoice_ids.filtered(lambda inv: inv.move_type == 'out_invoice')
