...
class WhatsappComposeInvoice(models.TransientModel):
    _name = 'whatsapp.compose.invoice'
    _description = 'WhatsApp Invoice Composer'
    ...
    message_text = fields.Text(
        ...
        default=lambda self: self.env['ir.config_parameter'].sudo().get_param(
            'acs_hms_whatsapp.default_invoice_message',
    ...
    def _onchange_invoice_id(self):
        """Update patient and phone when invoice changes"""
        if self.invoice_id and self.invoice_id.patient_id:
            self.patient_id = self.invoice_id.patient_id
            self.phone = self.invoice_id.patient_id.phone
    ...
    def _generate_invoice_pdf(self):
        """Generate invoice PDF and return attachment"""
        self.ensure_one()
        ...
        try:
            ...
            attachment = self.env['ir.attachment'].create({
                ...
                'description': f'Invoice PDF for WhatsApp - {self.invoice_id.name}',
            })

            # Ensure the attachment is committed to database before getting URL
            self.env.cr.commit()
            _logger.info(f"Created public attachment {attachment.id} for invoice {self.invoice_id.name}")

            return attachment
        except Exception as e:
            _logger.warning(f"Could not generate PDF: {str(e)}")
    ...
    def action_send_whatsapp(self):
        """Send invoice via WhatsApp to patient and emergency contacts"""
        self.ensure_one()
        ...
        try:
            _logger.info("Starting WhatsApp send process for invoice %s", self.invoice_id.name)
            # Generate or get attachment
            if self.send_as == 'pdf':
                attachment = self._generate_invoice_pdf()
            else:
            ...
            try:
                ...
                if formatted_patient_phone:
                    ...
                    _logger.info("Invoice sent to patient: %s", formatted_patient_phone)
            except Exception as e:
                    ...
                    try:
                        ...
                        if formatted_emergency_phone:
                            ...
                            emergency_message_text = _(
                                "Facture de %(patient_name)s (%(relationship)s)\n\n%(original_message)s"
                            ) % {
                            ...
                            _logger.info("Invoice sent to emergency contact: %s", formatted_emergency_phone)
                    except Exception as e:
            ...
            if sent_to:
                if attachment:
                    success_msg = _('Invoice sent successfully via WhatsApp to:\n') + '\n'.join([f"• {recipient}" for recipient in sent_to])
                else:
                    success_msg = _('Invoice sent successfully via WhatsApp to:\n') + '\n'.join([f"• {recipient}" for recipient in sent_to])
                    success_msg += _('\n\nNote: PDF attachment could not be generated. Message sent as text only.')
                ...
                raise UserError(_('Failed to send invoice to any recipient. Please check the phone numbers.'))
